...
- `TerrainPublication`: worker publication duration, CPU time and allocated bytes, with group/section counts and publication/failure flags. Include it explicitly in recordings that select events. It covers edit preparation and commit on the publication worker; frame capture can still contend with the atomic commit. Abandoned attempts are retained with `published: false`.

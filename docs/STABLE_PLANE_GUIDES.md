...
### Normal build with ordinary frame overlap — 2026-09-13

The normal production classpath at `b1ba89b9` completed the same 28-bundle matrix at 1280×720 with **ordinary frame overlap**, using a fresh disposable copy of the accepted 942-file fixture. All 364 EXRs pass hash/header/extent, same-frame, meaningful-channel finite and route/encoding checks. Temporal-NRD input validity, normal/roughness and hit-distance contracts pass; REBLUR has all nine per-plane stages plus SR in 158/158 recorded frames, RELAX in 198/198. Parent inspected all 16 corrected contact sheets, including transitions and restoration, and observed no persistent missing background or reflected side-wall endpoint. [Numerical review and limits](../tmp/normal-glass/review/results/FINAL_REVIEW.md), [contact sheets](../tmp/normal-glass/review/results/index.html), [normal-build launch](../tmp/normal-glass/manifest.json).

Clear transmission retains linear view Z **10.4960003 m** across all routes and both repeats. RR clear reflection retains view Z **10.652957/10.654196 m**; white reflection retains **10.656426/10.657170 m**, each with the side-wall normal approximately `(-0.999512,-0.003918,-0.003889)` and dominant plane 1. White transmission remains front-dominant around 5.500200 m. Under NRD the exported normal/view-Z inputs are still plane 0; an invalid clear-reflection center there is not evidence that the dominant reflected endpoint disappeared.

The new images contain foreground item geometry and NGX overlay marks absent from the reference. Noise/history and populations are unmatched. Two-capture noisy RR transmission means differ by roughly −10 to −13% RGB from the older two-capture reference, while reconstructed means differ by less than 0.66% in that case. This is not a radiance-equivalence test or an overlap comparison. The earlier longer SoA transport controls remain separate evidence. Saved settings and pose were restored, and all 942 original fixture files still match their pre-run hashes. [Parent audit](../tmp/normal-glass/PARENT_AUDIT.json).

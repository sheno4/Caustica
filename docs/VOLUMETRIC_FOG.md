...
Secondary transport evaluates the same captured spatial medium along reflected, transmitted and diffuse continuation segments. The engine integrates extinction and single scattering with adaptive Gauss–Legendre quadrature and one retained-light visibility sample per segment. Paths up to 1/4/8 metres use 1/2/4 samples respectively; longer paths use eight, capped at 256 metres. Explicit volume interiors use their existing absorption instead of outdoor fog. The primary post pass still owns the camera segment.
...
## Known limits and future quality work
...
The core fog feature and reference-scene validation are complete. The limits above bound the tested quality and identify further improvements; performance and visual claims remain specific to the recorded workloads.
...
Build adds incoming throughput times scattering, attenuates endpoint emission and child branches, and saves restart throughput after segment attenuation. Fill uses that saved throughput without repeating the restart segment's medium integration, scattering or emission. Later fill segments add scattering to their diffuse or specular signal. Scattering-weighted sample distances supply finite guides even when the segment ends at the sky. Depth-zero segments and explicit volume interiors are excluded.
...
## Adaptive secondary quadrature

The secondary integrator uses sorted Gauss–Legendre nodes and positive weights, treating their weighted extinction contributions as ordered optical intervals for source integration. This preserves the computed transmittance/scattering relation for the fog model's constant source-to-extinction ratio. The full eight-point rule reduces the 256-metre vertical exponential optical-depth errors above to approximately 0.0001% and 1.21%. It does not guarantee that all narrow or discontinuous authored media are resolved, and midpoint lighting remains a separate approximation.

A numerical sweep over 171,000 representative paths per density scale tested shorter rules and their switching thresholds. The retained rule uses one sample through 1 metre, two through 4 metres, four through 8 metres and eight beyond. At density scale 4, the largest short-path T error against dense integration was 0.000273; switching jumps at 1/4/8 metres were 0.000088/0.000180/0.000310, below 1/2048. Broader 2/8/64-metre thresholds were rejected for larger errors. The sweep covers smooth height profiles, ground exclusion, noise and 32-metre biome ramps, not arbitrary volume implementations or varying lighting. Some long grazing paths still have materially larger quadrature errors. Detailed cases and reference refinement are in `tmp/fog/adaptive-quadrature-analysis.py` and `.json`.

The initial full-eight-sample runtime candidate measured 50.45/50.80 FPS, with p95 frame intervals above 20 ms. Adaptive sampling measured 52.14/52.10 FPS, p95 19.62/19.69 ms and p99 19.83/19.91 ms at the reference 4K settings. All adaptive off/on endpoints had zero outstanding terrain builds after a longer warmup. Disabled repeats measured 59.41/59.45 FPS. Enabled mean GPU world tracing was 14.150/14.162 ms, including fill 9.542/9.566 ms; primary fog was 0.3867/0.3849 ms. The paired trace increase was 1.976/1.999 ms. Across separate runs, adaptive tracing was about 0.42 ms faster than full eight-sample tracing; differing warmup and workload prevent exact causal attribution. These measurements retain the 50 FPS target while improving the sampled thin-layer cases. Complete exports are in `tmp/fog/gauss8-benchmark/` and `tmp/fog/adaptive-benchmark/`.

Ray-tracing checks passed, including composed program compilation, SPIR-V validation, ABI/resource checks and generation reproducibility. Runtime motion captures also sampled pre-fog/pre-bloom reconstructed radiance, which includes secondary transport: all 80,870,400 RGB values across 13 bundles were finite and nonnegative. Sixteen primary-T captures stayed in [0.516602,0.979004]. Stationary mean T differences were 0.00013–0.00033, with localized maxima near 0.0757, comparable to the preceding full-eight-sample capture. Reviewed normal contacts show no obvious broad halo or corrupted frame; sparse moving stills do not establish flicker-free rendering. Results and contacts are in `tmp/fog/adaptive-motion/`.

Nsight captured and exported the adaptive implementation at 3840 × 2160 in `tmp/fog/nsight-adaptive/java_2026_09_13_21_37_25.ngfx-gputrace`. Three build intervals were 4.0395/6.5294/4.1160 ms; fill was 10.9960/10.3483/12.6745 ms; primary scene effects were 0.4205/0.4390/0.4128 ms, including material visibility 0.08365/0.08358/0.07638 ms. These are instrumented complete-stage timings with substantial build/fill variability, not isolated fog overhead or a stable-cadence result. Settled paired JFR remains the performance acceptance evidence.

The completion audit covers spatial density, controlled time/height/biome variation, coherent sampled motion, captured-data and coordinate contracts, current composed shader checks, the reference performance target, and debug/JFR/Nsight evidence. Earlier translation captures validate unchanged field/primary-coordinate mechanisms; their flight FPS is not represented as a benchmark of the adaptive shader. Approximate lighting, localized edge variation, difficult grazing paths and unattributed long-frame behavior remain explicitly bounded by the observations above.

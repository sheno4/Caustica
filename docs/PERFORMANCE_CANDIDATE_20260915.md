...
The cheaper next scheduling experiment must shorten closure lifetime across visibility without a new buffer. Simply moving BSDF evaluation before the shadow loop is insufficient: the caller still needs the same surface for `traceScatter` afterward. Preparing both the direct contribution and scatter continuation before visibility would let the closure leave live state. It requires explicit random-stream ownership and preserving emission, roulette, distance-guide, feedback, and per-plane accumulation semantics. This follow-up is not implemented yet.

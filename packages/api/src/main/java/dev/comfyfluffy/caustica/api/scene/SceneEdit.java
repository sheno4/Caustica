...
public sealed interface SceneEdit {
    ...
    record SetInstance<N>(InstanceId instance, SceneId scene, ReadyMesh<N> mesh,
        ...
        public SetInstance {
            Objects.requireNonNull(instance); Objects.requireNonNull(scene); Objects.requireNonNull(mesh);
            Objects.requireNonNull(transform); Objects.requireNonNull(instanceData); Objects.requireNonNull(primitiveLights);
    ...
    record DropInstance(InstanceId instance) implements SceneEdit {
        public DropInstance { Objects.requireNonNull(instance); }
    }
    ...
    record SetTransform(InstanceId instance, GeometryTransform transform, int mask) implements SceneEdit {
        public SetTransform {
            Objects.requireNonNull(instance); Objects.requireNonNull(transform);
            if ((mask & ~255) != 0) throw new IllegalArgumentException("visibility mask must fit in eight bits");
        }
    }
    record SetLight(LightId light, SceneId scene, LightDescriptor descriptor) implements SceneEdit {
        public SetLight { Objects.requireNonNull(light); Objects.requireNonNull(scene); Objects.requireNonNull(descriptor); }
    }
    record DropLight(LightId light) implements SceneEdit {
        public DropLight { Objects.requireNonNull(light); }
    }
    ...
    record SetEnvironment(SceneId scene, EnvironmentBinding<?> binding) implements SceneEdit {
        public SetEnvironment { Objects.requireNonNull(scene); Objects.requireNonNull(binding); }
    }
}

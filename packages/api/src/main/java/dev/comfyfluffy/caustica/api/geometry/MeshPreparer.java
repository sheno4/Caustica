 ...
 * Independent input claims are acquired before prepare returns and retained through preparation and by
 * the resulting revision. Cancelling the returned future abandons its result; accepted GPU work still
 * completes and its resources are released afterwards.
 */
public interface MeshPreparer {
     ...
     */
    <N> CompletableFuture<ReadyMesh<N>> prepare(
            ShaderDataType<N> instanceDataType, MeshBuild<N> build, ReadyMesh<N> refitSource);
}
